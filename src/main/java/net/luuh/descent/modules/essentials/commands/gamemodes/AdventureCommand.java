...
public class AdventureCommand extends ModuleCommand<Essentials> {
    ...
    protected void execute(Player player, String[] args) throws CommandSuccessException {
        ...
        if(!Permission.GAMEMODE_ADVENTURE.has(player)) throw new CommandSuccessException(false, helper.getRMUtils(), CommandsCaseExceptions.NO_PERMS, player);
        if (args.length > 0) {
            if(Bukkit.getPlayer(args[0]) == null && Bukkit.getPlayer(args[0]).isOnline()) throw new CommandSuccessException(false, helper.getRMUtils(), CommandsCaseExceptions.INVALID_PLAYER, player);
